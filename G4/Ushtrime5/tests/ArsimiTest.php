...
class ArsimiTest extends PHPUnit_Framework_TestCase
{
	...
	public function testCheckPageContent()
    {
        $this->session->visit('http://www.arsimi.gov.al');

        $this->session->getPage()->find('css', '#dropdown6 .subnav li a')->click();

        $hasVideo = $this->session->getPage()->hasContent('Video');

        $this->assertTrue($hasVideo);
    }
